...
exports.getCommissionList = function(req, res) {
    if (req.url == '/getCommissionList') {
        ...
        m_httpUtils.post_receive(req, function(recvData, tag) {
            ...
            m_db.find_commission_list(sql_obj, function(orderList) {
                res_data["list"] = orderList;
                m_db.find_order_sum_cost(sql_obj, function(allSumCostData) {
                    if (allSumCostData && allSumCostData.length > 0) res_data["all_totle_commission"] = allSumCostData[0]["all_totle_commission"];
